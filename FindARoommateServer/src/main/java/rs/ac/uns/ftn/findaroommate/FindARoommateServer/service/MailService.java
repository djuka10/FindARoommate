...
public class MailService {
	...
	public void send(EmailDto email) throws MessagingException {
		...
		helper.setSubject("FindARoommate notification:");
		helper.setFrom(email.getSendFrom()); 
		mailSender.send(mimeMessage);
		System.out.println("saljem mejl"); 
	}
	
	private EmailDto prepareMessage(EmailDto email) {
		String sendFrom = env.getProperty("spring.mail.username");
		String content = generateMessage(email.getSubject(), email.getContent(), email.getAdditionalInfo());
			
		EmailDto mail = new EmailDto(email.getSubject(), content, "text/html", email.getSendTo(), sendFrom, false, "utf-8", "");
		return mail;
	}
